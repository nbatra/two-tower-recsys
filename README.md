...
The `data/` and `models/` directories are excluded from version control due to size (~1GB extracted dataset, ~500MB trained artifacts). To regenerate:

1. **Seed data:** Run `./setup.sh` which downloads MovieLens 25M (~250MB compressed, ~1GB extracted) from https://files.grouplens.org/datasets/movielens/ml-25m.zip into `data/`.
2. **Processed features:** Notebooks 01-02 compute feature parquets in `data/processed/`.
3. **Models:** Training notebooks generate model artifacts in `models/`:
   - NB03 trains the Two-Tower retrieval model + extracts embeddings + builds FAISS index
   - NB04 trains the XGBoost LambdaMART ranker (Two-Tower pipeline)
   - NB06 trains the ComiRec multi-interest retrieval model
   - NB07 trains the XGBoost LambdaMART ranker (ComiRec pipeline)
   - NB09 trains the SASRec sequential retrieval model
   - NB10 trains the XGBoost LambdaMART ranker (SASRec pipeline)

Run notebooks sequentially (00-13) on first setup. Full pipeline takes approximately 2-3 hours on an M4 Max.
