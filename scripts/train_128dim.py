"""
Approach F: 128-dim output embeddings + wider MLP.

Hypothesis: 64 dimensions may not be enough to encode preferences for 21K items
and 138K users. With 128-dim embeddings:
- FAISS has more dimensions to separate items
- User embeddings can encode more nuanced taste profiles
- Cosine similarity in higher dimensions is more discriminative

Architecture: same wider MLP as Approach E but output_dim=128.
Same training strategy (69M BCE, random negatives, OneCycleLR).
"""
import numpy as np
import pandas as pd
import pickle
...
print(f'Output dim: {OUTPUT_DIM} (baseline was 64)', flush=True)
...
print('\nStarting 128-dim training...', flush=True)
print('=' * 70, flush=True)
...
for epoch in range(EPOCHS):
    ...
    if val_auc > best_auc:
        ...
        best_epoch = epoch + 1
        patience_counter = 0
        torch.save(model.state_dict(), MODEL_DIR / 'two_tower_128dim.pt')
...
with open(MODEL_DIR / 'history_128dim.pkl', 'wb') as f:
    pickle.dump(history, f)

# ============================================================
# 4. Extract embeddings and evaluate
# ============================================================
model.load_state_dict(torch.load(MODEL_DIR / 'two_tower_128dim.pt', map_location=device))
model.eval()
...
np.save(MODEL_DIR / 'item_embeddings_128dim.npy', item_embeddings)
np.save(MODEL_DIR / 'user_embeddings_128dim.npy', user_embeddings)

index = faiss.IndexFlatIP(OUTPUT_DIM)
index.add(item_embeddings)
faiss.write_index(index, str(MODEL_DIR / 'faiss_index_128dim.bin'))
...
print(f'\n128-dim Recall@K:', flush=True)
for k in K_values:
    print(f'  Recall@{k:<4}: {np.mean(recalls[k]):.4f}', flush=True)

print(f'\nBaseline (64-dim) Recall@K:', flush=True)
...
print(f'\nWider MLP (64-dim) Recall@K:', flush=True)
print(f'  Recall@10  : 0.0161', flush=True)
print(f'  Recall@50  : 0.0856', flush=True)
print(f'  Recall@100 : 0.1608', flush=True)
print(f'  Recall@200 : 0.2776', flush=True)
print(f'  Recall@500 : 0.4817', flush=True)
