"""
Approach B: Regularized model with smaller embeddings and explicit L2 penalty.

Key differences from baseline:
- Smaller embedding dim (32 vs 64) to reduce overfitting
- Explicit L2 regularization on embedding weights (not just weight_decay)
- Lower dropout (0.1) - smaller model needs less regularization from dropout
- More epochs (8) with lower LR and cosine schedule
- Same 69M training pairs with BCE loss
"""
import numpy as np
import pandas as pd
import pickle
...
# ============================================================
# 2. Model - smaller, more regularized
# ============================================================
EMBEDDING_DIM = 32
HIDDEN_DIM = 128
OUTPUT_DIM = 64
...
class TwoTowerModel(nn.Module):
    ...
    def embedding_reg_loss(self):
        user_emb_weight = self.user_tower.user_embedding.weight[1:]
        item_emb_weight = self.item_tower.item_embedding.weight[1:]
        return (user_emb_weight.norm(2).pow(2) + item_emb_weight.norm(2).pow(2)) / (user_emb_weight.shape[0] + item_emb_weight.shape[0])
...
print(f'Embedding reg lambda: {EMB_REG_LAMBDA}', flush=True)

best_auc = 0.0
best_epoch = 0
history = []

print('\nStarting regularized training...', flush=True)
print('=' * 70, flush=True)
...
for epoch in range(EPOCHS):
    ...
    for batch_idx in range(n_batches_train):
        ...
        bce_loss = criterion(scores, labs)
        reg_loss = model.embedding_reg_loss()
        loss = bce_loss + EMB_REG_LAMBDA * reg_loss
    ...
    if val_auc > best_auc:
        best_auc = val_auc
        best_epoch = epoch + 1
        torch.save(model.state_dict(), MODEL_DIR / 'two_tower_regularized.pt')
        print(f'  --> Best model saved (AUC={best_auc:.4f})', flush=True)

print(f'\nBest Val AUC: {best_auc:.4f} at epoch {best_epoch}', flush=True)

with open(MODEL_DIR / 'history_regularized.pkl', 'wb') as f:
    pickle.dump(history, f)

# ============================================================
# 4. Extract embeddings and build FAISS
# ============================================================
model.load_state_dict(torch.load(MODEL_DIR / 'two_tower_regularized.pt', map_location=device))
model.eval()
...
np.save(MODEL_DIR / 'item_embeddings_regularized.npy', item_embeddings)
np.save(MODEL_DIR / 'user_embeddings_regularized.npy', user_embeddings)

index = faiss.IndexFlatIP(OUTPUT_DIM)
index.add(item_embeddings)
faiss.write_index(index, str(MODEL_DIR / 'faiss_index_regularized.bin'))
...
print(f'\nRegularized Recall@K:', flush=True)
for k in K_values:
...
print('\nDone! (Regularized model)', flush=True)
