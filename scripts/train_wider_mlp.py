"""
Approach E: Same training strategy as baseline, but wider/deeper MLP.

Key insight from failed approaches A-D:
- The baseline's TRAINING STRATEGY is correct (69M BCE, random negatives, early stopping)
- Every attempt to change the loss, negatives, or fine-tune degraded performance
- The untested lever: MODEL CAPACITY in the MLP layers

Baseline MLP: 88->128->64->64 (user), 137->128->64->64 (item)
This model:   88->256->256->128->64 (user), 137->256->256->128->64 (item)

The theory: with a wider MLP, the model can learn richer non-linear interactions
between ID embeddings and side features. The baseline's narrow bottleneck (128-dim)
might limit how well it can combine "this user's collaborative signal" with
"this user's genre preferences."

Also: 10 epochs with patience-based early stopping (not fixed 5).
"""
import numpy as np
import pandas as pd
import pickle
...
print(f'Model parameters: {total_params:,} (baseline was 10.2M)', flush=True)

# ============================================================
# 3. Training (same strategy as baseline)
# ============================================================
BATCH_SIZE = 8192
EPOCHS = 10
LOG_EVERY = 2000
...
print(f'Max epochs: {EPOCHS}, Early stopping patience: {PATIENCE}', flush=True)
...
print('\nStarting wider MLP training...', flush=True)
print('=' * 70, flush=True)
...
for epoch in range(EPOCHS):
    ...
    if val_auc > best_auc:
        ...
        best_epoch = epoch + 1
        patience_counter = 0
        torch.save(model.state_dict(), MODEL_DIR / 'two_tower_wider.pt')
...
with open(MODEL_DIR / 'history_wider.pkl', 'wb') as f:
    pickle.dump(history, f)

# ============================================================
# 4. Extract embeddings and evaluate
# ============================================================
model.load_state_dict(torch.load(MODEL_DIR / 'two_tower_wider.pt', map_location=device))
model.eval()
...
np.save(MODEL_DIR / 'item_embeddings_wider.npy', item_embeddings)
np.save(MODEL_DIR / 'user_embeddings_wider.npy', user_embeddings)

index = faiss.IndexFlatIP(OUTPUT_DIM)
index.add(item_embeddings)
faiss.write_index(index, str(MODEL_DIR / 'faiss_index_wider.bin'))
...
print('\nDone! (Wider MLP model)', flush=True)
